...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && req.url === '/api/session') { const s = getSession(sessionId(req)); if (!s) return json(res, 401, { error: '需要訪問碼' }); let current = null; const last = [...s.messages].reverse().find(m => m.role === 'assistant'); try { current = last ? JSON.parse(last.content) : null; } catch {} return json(res, 200, { turns: s.turns, maxTurns, current }); }
    if (req.method === 'POST' && req.url === '/api/interview') { if (!rateLimit(req, res, 'interview', 30, 60 * 60 * 1000)) return; const id = sessionId(req), s = getSession(id); if (!s) return json(res, 401, { error: '會話已失效' }); if (s.turns >= maxTurns) return json(res, 429, { error: '本次訪談已達最大題數' }); const b = await body(req), answer = String(b.answer || '').trim(); if (answer.length > 6000) return json(res, 400, { error: '回答內容過長，請濃縮後再送出' }); if (s.messages.length && answer.length < 2) return json(res, 400, { error: '請寫下你的真實想法後再繼續' }); const messages = [...s.messages]; if (answer) messages.push({ role: 'user', content: answer }); const result = await complete(messages.slice(-40), s.turns); updateSession(id, x => { if (answer) x.messages.push({ role: 'user', content: answer }); x.messages.push({ role: 'assistant', content: JSON.stringify(result) }); x.turns += result.type === 'question' ? 1 : 0; }); return json(res, 200, result); }
