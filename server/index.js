...
const { claimDailySlot, loginWithCode, getSession, updateSession, updateConsent, consentOf, availability } = require('./store');
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && req.url === '/api/access/claim') { if (!rateLimit(req, res, 'claim', 8, 24 * 60 * 60 * 1000)) return; const b = await body(req); if (b.serviceConsent !== true) return json(res, 400, { error: '請先閱讀並同意完成訪談所需的資料保存說明' }); const issued = claimDailySlot(limit, ttl, b.researchConsent === true); setSession(res, issued.sessionId); return json(res, 200, { code: issued.code, remaining: issued.remaining, consent: consentOf(getSession(issued.sessionId)) }); }
    ...
    if (req.method === 'GET' && req.url === '/api/session') { const s = getSession(sessionId(req)); if (!s) return json(res, 401, { error: '需要訪問碼' }); let current = null; const last = [...s.messages].reverse().find(m => m.role === 'assistant'); try { current = last ? normalizeResult(JSON.parse(last.content)) : null; } catch {} return json(res, 200, { turns: s.turns, maxTurns, current, consent: consentOf(s) }); }
    if (req.method === 'POST' && req.url === '/api/consent') { const id = sessionId(req), s = getSession(id); if (!s) return json(res, 401, { error: '會話已失效' }); const b = await body(req), updated = updateConsent(id, b.researchConsent === true); return json(res, 200, { consent: consentOf(updated) }); }
    if (req.method === 'POST' && req.url === '/api/interview') { if (!rateLimit(req, res, 'interview', 80, 60 * 60 * 1000)) return; const id = sessionId(req), s = getSession(id); if (!s) return json(res, 401, { error: '會話已失效' }); const b = await body(req), answer = String(b.answer || '').trim(); if (answer.length > 6000) return json(res, 400, { error: '回答內容過長，請濃縮後再送出' }); if (s.messages.length && answer.length < 2) return json(res, 400, { error: '請寫下你的真實想法後再繼續' }); const messages = [...s.messages]; if (answer) messages.push({ role: 'user', content: answer }); let result; try { result = await complete(messages.slice(-40), s.turns); } catch (error) { updateSession(id, x => { x.lastError = { at: Date.now(), type: 'generation' }; }); throw error; } if (s.turns >= maxTurns && result.type !== 'report') throw new Error('达到最大题数时模型未能生成报告'); updateSession(id, x => { if (answer) x.messages.push({ role: 'user', content: answer }); x.messages.push({ role:'assistant', content:JSON.stringify(result) }); x.turns += result.type === 'question' ? 1 : 0; delete x.lastError; }); return json(res, 200, result); }
