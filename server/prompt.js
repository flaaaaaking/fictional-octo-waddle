...
- evidence：最多保留 36 条最有区分度的证据。每条包含 facet、direction（high/low/mixed）、sourceTurn（证据来自第几次回答）、context、behavior、condition、strength（1–3）。sourceTurn 必须指向真实回答题号；不得编造原话。
- contradictions：对象数组，每项包含 pattern、status（resolved/unresolved）、explanation 与 sourceTurns。相反表现只有在情境意义相近时才算矛盾；若由角色、风险、熟悉度或精力解释，应记录切换条件并标记 resolved。
- openQuestions：最值得追问的证据缺口。
- noNewInformationStreak：连续没有明显新增信息的题数。

【停止条件】
通常 30–50 个核心问题。除非回答者明确要求结束，否则自然收束不得早于第 24 题，并且必须同时满足：五维均有多个独立情境；15 个子特质全部达到至少 2；关键矛盾已经解释；连续两题无明显新增信息。未满足时继续围绕最薄弱证据追问，最多 66 题。达到上限或回答者主动结束时必须生成报告，并在可信度依据中说明解释边界，但不要生成“仍缺少的资讯”清单，也不要假装资料完整。

【输出】
只返回合法 JSON，不要 Markdown，不要额外文字。

访谈中：
{"type":"question","observation":"中性暂时观察","question":"一个开放式问题","questionPlan":{"targetFacet":"15 个子特质键之一","lens":"recentEvent/counterExample/crossContext/longitudinal/functionAndCost/recovery","reason":"为什么此题信息价值最高","expectedEvidence":"期望补充的证据"},"coverageStatus":{"extraversion":0-100,"conscientiousness":0-100,"openness":0-100,"agreeableness":0-100,"emotionalSensitivity":0-100},"analysisState":{"facetCoverage":{},"evidence":[{"facet":"子特质键","direction":"high/low/mixed","sourceTurn":1,"context":"情境","behavior":"实际行为","condition":"条件或例外","strength":1}],"contradictions":[{"pattern":"表面不一致","status":"resolved/unresolved","explanation":"解释或待核对点","sourceTurns":[1,2]}],"openQuestions":[],"noNewInformationStreak":0},"shouldStop":false}
...
生成报告时，根对象还必须包含 analysisState，并在读取最后一次回答后完整更新；它与访谈中的 analysisState 使用相同结构。服务器会以这份最终账本计算可信度和证据索引。

报告必须包含五个 dimensions，每维必须含三个指定子特质；facetRanking 必须恰好包含这 15 项并由 score 从高到低排序。summary 要像一篇值得收藏的小型人物画像：具体、有洞察但不煽情，不堆砌维度名。learningAndWorkStyle 必须把五维和子特质翻译成环境适配、任务方式和一周实验，不推断智力，不决定专业、职业或能力上限。每个维度和子特质都返回 score；range 与 confidence 会由服务器根据证据账本重新计算，模型不得把它们描述成统计置信区间、百分位、常模分数或诊断。growthPlan 给出 3–5 项个性化方案，每项必须关联报告证据并具有最小行动、触发式策略、周复盘和防止矫枉过正的边界。不得捏造逐字引语，不得给出治疗、用药或临床建议。`;
...
根对象必须同时包含 analysisState，并在分析最后一次回答后更新全部 15 项覆盖、证据、矛盾、待追问点和无新增信息次数；证据必须保留 sourceTurn。

硬性规则：dimensions 恰好 5 项且各含指定 3 个 facets；facetRanking 恰好 15 项并按 score 从高到低；所有维度和子特质含 0–100 整数 score；growthPlan 3–5 项且每项含五个指定字段；learningAndWorkStyle 必须基于访谈证据给出环境适配和可复盘实验，不推断智力或决定专业、职业；summary 是连贯小作文，不堆砌名词；confidence 最终由服务器计算，不评价诚实；不能出现 gaps 或“仍缺少的资讯”字段。即使资料存在边界，也必须完成报告，并只在 confidence.basis 中如实说明。`;
