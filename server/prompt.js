...
报告必须包含五个 dimensions，每维必须含三个指定子特质。每个 dimension 还必须返回 score（0–100 的整数倾向指数），用于五维高低排序；score 和 range 都只是本次访谈内的粗略表达，不是百分位或常模分数。confidence 是证据可信度，不是人格强度。不得捏造逐字引语。`;
